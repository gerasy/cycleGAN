...
def get_prediction_matrix( model, param_list, pre_calculated=False, source_domain = "A",dataBaseName = "horse2zebra"):
    if not pre_calculated:
        ...
        org, gen, org_percentage, gen_percentage = model.eval_testset(param_eval_testset, source_domain= source_domain, pic_number_low=low_pic, pic_number_high=high_pic,
...
def plot_generated_images(model, img_idx, param_list, source_domain = "A"):
    ...
    model.load_cycle_nets(epoch = 100, model_name = param_list[0].name)
    org, gen, org_percentage, gen_percentage = model.eval_testset(param_eval_testset, source_domain= source_domain, pic_number_low=img_idx, pic_number_high=img_idx,
                                                                pre_generated = False,  plot = False, explain = False)

    loss = calc_background_loss(model = model , source_domain = source_domain, img_idx = img_idx, masks_precalculated = True, plot = False)
    ...
    plt.xticks([])
    plt.yticks([])
    plt.title("generated image \n {} \n pred. score: {} \n background err: {}  ".format(param_list[0].name, gen_percentage[0], loss ))
    ...
    for i in range(1, len(param_list)):
        ...
        plt.xticks([])
        plt.yticks([])
        plt.title("generated image \n {} \n pred. score: {} \n background err: {}  ".format(param_list[i].name, gen_percentage[0], loss ))
...
def get_background_loss_matrix(model, param_list, pre_calculated=False, source_domain = "A", dataBaseName="horse2zebra", epochToLoad = 100):
    ...
    if not pre_calculated:
        ...
        for idx in good_list: 
            temp = calc_background_loss(model, source_domain = source_domain, img_idx = idx, masks_precalculated = True, plot = False)
            losses.append(temp)
...
def estimate_relevance_location(model, source_domain = "A", img_idx = 0, masks_precalculated = True, plot = True):
    ...
    o_attributions_A, o_attributions_B, g_attributions_A, g_attributions_B = create_explanation(original, generated, model.classifier, algorithm= "saliency")
...
def get_attribution_ratio_matrix(model, param_list, pre_calculated=False, source_domain = "A"):
    ...
    if not pre_calculated:
        ...
        for idx in good_list: 
            temp_tuple = estimate_relevance_location(model = model, source_domain = source_domain, img_idx = idx, masks_precalculated = True, plot = False)
        ...
        np.save("./checkpoints/horse2zebra/attribution_ratio_matrix_{}".format(source_domain), attribution_ratio_matrix)
    
    else: # if precalculated
        attribution_ratio_matrix = np.load("./checkpoints/horse2zebra/attribution_ratio_matrix_{}.npy".format(source_domain), allow_pickle=True)
...
def plot_attribution_ratio_matrix(matrix, param_list, source_domain= "A" ):
    ...
    plt.xlabel("attribution scores for original and generated images w.r.t. to different classes. Source domain: {}".format(source_domain))
    plt.tight_layout()
